...
class WindowsUtils(base.BaseOSUtils):
    ...
    def _set_static_network_config(name, address, prefix_len, gateway):
        ...
        try:
            ...
            if "." in address:
                try:
                    LOG.debug("Setting IPv4 network manually")
                    LOG.debug(address)
                    cidr = int(prefix_len)
                    mask = (0xffffffff >> (32 - cidr)) << (32 - cidr)
                    subnet = (str( (0xff000000 & mask) >> 24) + '.' +
                            str( (0x00ff0000 & mask) >> 16) + '.' +
                            str( (0x0000ff00 & mask) >> 8)  + '.' +
                            str( (0x000000ff & mask)))
                    os.system("netsh interface ipv4 set address name=\"%s\" static %s %s %s" %(name, address, subnet, gateway))
                except Exception as ex:
                    LOG.exception(ex)
            else:
                try:
                    LOG.debug("Setting IPv6 network manually")
                    LOG.debug(address)
                    os.system("netsh interface ipv6 set global randomizeidentifiers=disabled")
                    os.system("netsh interface ipv6 add address \"%s\" %s/%s store=persistent" %(name, address, prefix_len))
                    os.system("netsh interface ipv6 add route ::/0 \"%s\" %s store=persistent" %(name, gateway))
                except Exception as ex:
                    LOG.exception(ex)
